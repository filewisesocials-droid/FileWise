...
export const TrackingView: React.FC<TrackingViewProps> = ({ initialTrackId = 'FW-98214', onStartNewInquiry }) => {
  ...
  return (
    <section className="py-12 bg-slate-50 text-slate-900 min-h-[700px] font-sans">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Title */}
        <div className="text-center max-w-2xl mx-auto mb-8">
          <div className="inline-flex items-center gap-2 bg-amber-100 border border-amber-200 text-amber-900 text-xs font-bold px-3 py-1 rounded shadow-2xs mb-3">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm mb-8">
          <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:border-slate-800 focus:bg-white font-mono font-medium"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs px-6 py-3 rounded-lg shadow-md transition-all flex items-center justify-center gap-2"
            >
          ...
          <div className="mt-3 flex items-center gap-2 text-xs text-slate-500">
            ...
            <button
              ...
              className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-mono font-bold px-2.5 py-1 rounded border border-slate-200"
            >
              FW-98214 (SA to UAE)
            </button>
        ...
        {/* Error message */}
        {errorMsg && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center text-amber-900 space-y-2 mb-8">
            <AlertCircle className="w-8 h-8 text-amber-600 mx-auto" />
            <h3 className="font-serif font-bold text-slate-900 text-base">Order Not Found</h3>
        ...
        {currentOrder && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 sm:p-8 shadow-sm space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-100">
              <div>
                <div className="flex items-center gap-3">
                  <span className="text-xs font-mono font-bold bg-slate-900 text-amber-400 px-3 py-1 rounded border border-slate-800">
                    REF: {currentOrder.id}
                  </span>
                  <span className="text-xs text-slate-500">Logged: {currentOrder.createdDate}</span>
                </div>
                
                <h3 className="text-xl font-serif font-bold text-slate-900 mt-2">
                  {currentOrder.originCountry} <ArrowRight className="inline w-4 h-4 text-amber-600 mx-1" /> {currentOrder.destinationCountry}
                </h3>
              ...
              <div className="text-right">
                ...
                <span className="text-xs font-bold text-slate-900 bg-amber-50 px-3 py-1 rounded border border-amber-200 inline-block mt-1">
            ...
            <div>
              <h4 className="text-xs font-serif font-bold uppercase tracking-wider text-slate-500 mb-6">
                  ...
                  <div key={idx} className="relative group">
                    ...
                    <div className={`absolute -left-[31px] top-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border ${
                      step.completed
                        ...
                        : step.current
                        ? 'bg-slate-900 text-amber-400 border-slate-900 ring-4 ring-amber-100'
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-1">
                        <h5 className={`font-bold text-sm ${step.current ? 'text-slate-900' : step.completed ? 'text-slate-900' : 'text-slate-400'}`}>
                          {step.title}
                        </h5>
                        {step.date && (
                      ...
                      {step.location && (
                        <p className="text-xs text-amber-800 flex items-center gap-1 mt-1 font-semibold">
            ...
            <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
              <div className="space-y-1">
                <span className="text-slate-500 block">Courier Delivery Details:</span>
                <span className="font-mono text-slate-900 font-bold">{currentOrder.courierName} • #{currentOrder.trackingNumber}</span>
              </div>

              <div className="flex items-center gap-3">
                <button
                  onClick={() => alert(`Verification certificate for ${currentOrder.id} generated.`)}
                  className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold px-4 py-2.5 rounded-lg border border-slate-200 flex items-center gap-1.5 transition-colors"
                >
                  <Download className="w-4 h-4 text-slate-600" /> Download Status Report
                </button>
              </div>
            </div>
